...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func setupFirebase() {
        guard let path = Bundle.main.path(forResource: "GoogleService-Info", ofType: "plist") else {
            GCLogger.error("GoogleService-Info.plist not found")
            return
        }
        ...
        guard let plist = NSDictionary(contentsOfFile: path) as? [String: Any],
            ...
            GCLogger.error("GOOGLE_APP_ID not found in GoogleService-Info.plist")
            return
        }

        GCLogger.info("Firebase configured")
...
extension AppDelegate: UNUserNotificationCenterDelegate, MessagingDelegate {
    func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        ...
        GCLogger.info("Device token registered: \(private: apnsToken)")
    ...
    func application(_ application: UIApplication, didFailToRegisterForRemoteNotificationsWithError error: any Error) {
        GCLogger.error("Remote notification registration failed: \(public: error.localizedDescription)")
        
        ToastManager.shared.showToast(message: "Failed to register: \(error.localizedDescription)")
    }
    
    func application(_ application: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable: Any],fetchCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) {
        GCLogger.info("Push notification received")
